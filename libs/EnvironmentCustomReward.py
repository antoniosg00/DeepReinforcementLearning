...
class StockEnv(gym.Env):
    ...
    def __init__(self, df, n_obs=10, transaction_rate=0.01, initial_worth=1000, render_mode=None):
        ...
        self.transaction_rate = transaction_rate # Commission on any transaction
    ...
    def _get_info(self):
        # Obtaining the attached information
        info_dict = {
    ...
    def _exe_action(self, action):
        ...
        # We add net worth in this step to the historical record
        self.net_worth_historial.append(self.net_worth)

    def _get_reward(self):
        # Obtaining the agent's reward (mix of ideas -> improvable)
        N1 = 5 # long_term 1
        N2 = 10 # long_term 2
        reward = 0

        # We calculate change in short-term and long-term net worth
        short_worth_delta = self.net_worth_historial[-1] - self.net_worth_historial[-2]
        if len(self.net_worth_historial) >= N1: long1_worth_delta = self.net_worth_historial[-1] - self.net_worth_historial[-N1]
        if len(self.net_worth_historial) >= N2: long2_worth_delta = self.net_worth_historial[-1] - self.net_worth_historial[-N2]
        
        # We benefit short- and long-term strategy (as a %)
        reward += short_worth_delta / self.net_worth_historial[-2] * 100 # Percentage of growth
        if len(self.net_worth_historial) >= N1: reward += long1_worth_delta / self.net_worth_historial[-N1] * 100
        if len(self.net_worth_historial) >= N2: reward += long2_worth_delta / self.net_worth_historial[-N2] * 100

        # We introduce a term to control risk (we penalize investment above 50% for risk).
        risk_penalty = 10 * max(0, self.shares_held * self.df['Close'].iloc[self.current_step] / self.net_worth_historial[-1] - 0.5)
        reward -= risk_penalty
    ...
    def step(self, action):
        ...
        # If the last instance of the data tensor is reached, it returns to the first instance without resetting the counter self.current_step
        ...
        # We truncate the episode if a minimum return of 1% has not been achieved in one year.
        truncated = False
        if len(self.net_worth_historial) % 365 == 0: # Once a year
            if (self.net_worth_historial[-1] - self.net_worth_historial[-365])/self.net_worth_historial[-365] * 100 < 1.:
                truncated = True
