...
class extract_tensor(nn.Module):
    # Thanks to https://stackoverflow.com/questions/44130851/simple-lstm-in-pytorch-with-sequential-module
    def forward(self,x):
...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict):
        ...
        lstm_input_features = history_subspace.shape[0]
        length_of_sequence = history_subspace.shape[1]

        # We define the extractor for the subspace 'history' (LSTM)
        history_extractor = nn.Sequential(
            nn.LSTM(input_size=lstm_input_features, hidden_size=64, num_layers=2, batch_first=True, dropout=0.5, bidirectional=True), 
            extract_tensor(),
            # Impossible Batch normalization because it uses batches of N=1 examples (std null -> division by 0) https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm1d.html
            nn.Linear(64 * 2, 128),  # Adjustment of the input size according to the LSTM output (factor 2 due to bidirectionality)
            nn.ReLU(),
            nn.Dropout(0.5) 
        )
    ...
    def forward(self, observations) -> th.Tensor:
        ...
        for key, extractor in self.extractors.items():
            if key=='history': observations[key] = observations[key].permute(0, 2, 1) # Ver: https://pytorch.org/docs/stable/generated/torch.nn.LSTM.html
...
class CustomNetwork(nn.Module):
    ...
    :param feature_dim: dimension of the features extracted with the features_extractor (e.g. features from a LSTM or CNN)
...
class CustomActorCriticPolicy(MultiInputActorCriticPolicy): # We use Adam as default optimizer
    def __init__(
        self,
